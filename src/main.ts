...
import {
  ScratchAuthCookie_eraseEncryptedData,
  ScratchAuthCookie_getDecryptedSessionId,
  ScratchAuthCookie_setEncryptedData,
} from "./cookie/clientUtils";
...
export async function ScratchAuthGET_session() {
  try {
    const session = await ScratchAuthCookie_getDecryptedSessionId(
      "scratchauth-session"
    );
    return session;
  } catch (error) {
  ...
  try {
    if (privateCode) {
      ...
      if (res) {
        ...
        await ScratchAuthCookie_setEncryptedData(
          "scratchauth-session",
          obj.data.username,
          expiration
        );
