...
// データをセッションに保存する関数
export async function ScratchAuthCookie_setEncryptedData(
  content: string,
  value: string,
  days: number
) {
...
export async function ScratchAuthCookie_getDecryptedSessionId(
  content: string
): Promise<string | null> {
  const encryptedValue = ScratchAuthCookie_getCookie(content); // Cookieから値を取得
  if (encryptedValue) {
    const [sessionId, hmac] =
      ScratchAuthCookie_decrypt(encryptedValue).split("|"); // セッションIDとHMACに分割
...
export async function ScratchAuthCookie_setEncryptedSessionId(
  content: string,
  value: string,
  days: number
) {
